...
#DEFINE PUTDOWN RULES
putdown_rules = []

for disk in disks:
   for peg in pegs:
      for t in time_points:
            action = f"put-down_{disk}_{peg}_{t}"
            conditions = []
            ...
            # Combine all conditions into a single string with indentation
            rule = f"(:implies {action} (and {' '.join(conditions)}))"
            putdown_rules.append('\n' + rule)

for t in time_points:
    action = f"put-down_d1_d2_{t}"
    conditions=[]
...
for rule in putdown_rules:
    print(rule)
print(len(putdown_rules))

print(len(putdown_rules) + len(pickup_rules))
